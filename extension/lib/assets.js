...
api.fetchText = function(url, onLoad, onError) {
    ...
    var contentLoaded = 0,
        timeoutAfter = pm.hiddenSettings.assetFetchTimeout * 1000 || 30000,
        timeoutTimer,
    ...
    var onErrorEvent = function() {
        cleanup();
        pm.logger.writeOne('', 'error', errorCantConnectTo.replace('{{msg}}', actualUrl));
...
var getAssetSourceRegistry = function(callback) {
    ...
    var createRegistry = function() {
        api.fetchText(
            pm.assetsBootstrapLocation || 'assets/assets.json',
            function(details) {
...
var readUserAsset = function(assetKey, callback) {
    ...
    var toRead = assetKey;
    if ( assetKey === pm.userFiltersPath ) {
        toRead = [
            assetKey,
...
api.get = function(assetKey, options, callback) {
    console.log('assets.get: '+assetKey)
