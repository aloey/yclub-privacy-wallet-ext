...
pm.dateNowToSensibleString = function() {
...
pm.LineIterator = function(text, offset) {
    this.text = text;
    this.textLen = this.text.length;
    this.offset = offset || 0;
};

pm.LineIterator.prototype.next = function(offset) {
    if ( offset !== undefined ) {
        this.offset += offset;
    }
...
pm.LineIterator.prototype.charCodeAt = function(offset) {
    return this.text.charCodeAt(this.offset + offset);
};

pm.LineIterator.prototype.eot = function() {
...
pm.FieldIterator.prototype.first = function(text) {
    this.text = text;
    this.offset = 0;
    return this.next();
};

pm.FieldIterator.prototype.next = function() {
...
pm.FieldIterator.prototype.remainder = function() {
    return this.text.slice(this.offset);
};

/******************************************************************************/

pm.CompiledLineWriter = function() {
    this.output = [];
    this.stringifier = JSON.stringify;
};

pm.CompiledLineWriter.fingerprint = function(args) {
    return JSON.stringify(args);
};

pm.CompiledLineWriter.prototype = {
    push: function(args) {
...
pm.CompiledLineReader = function(raw) {
    this.reset(raw);
    this.parser = JSON.parse;
};

pm.CompiledLineReader.prototype = {
    reset: function(raw) {
        this.input = raw;
        this.len = raw.length;
        this.offset = 0;
...
pm.arrayFrom = typeof Array.from === 'function'
    ? Array.from
    : function(iterable) {
        var out = [], i = 0;
...
pm.openNewTab = function(details) {
