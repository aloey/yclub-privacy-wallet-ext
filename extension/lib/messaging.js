...
(function () {
    ...
    var onMessage = function (request, sender, callback) {
        ...
        if (pm.cloudStorageSupported !== true) {
            callback();
            return;
        }
        ...
        switch (request.what) {
            case 'cloudGetOptions':
                vAPI.cloud.getOptions(function (options) {
                    options.enabled = pm.userSettings.cloudStorageEnabled === true;
                    callback(options);
                });
                return;
    ...
    var onMessage = function (request, sender, callback) {
        ...
        switch (request.what) {
            case 'temporarilyWhitelistDocument':
                pm.webRequest.temporarilyWhitelistDocument(request.hostname);
                break;
